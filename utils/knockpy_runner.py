...
def runner_knockpy(domain, save_path):
    ...
    print(f"[+] Found {len(results)} subdomains. Starting cert collection...")

    for idx, entry in enumerate(results):
        subdomain = entry.get("domain")
        yield f"    ↳ Fetching cert for {subdomain} ({idx + 1}/{len(results)})"
        print(f"    ↳ Fetching cert for {subdomain} ({idx + 1}/{len(results)})")
    ...
    existing_data[domain]["scan_timestamp"] = datetime.now().isoformat() + "Z"
